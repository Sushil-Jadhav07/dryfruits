...
const Product: React.FC<ProductProps> = ({ data, type, style }) => {
    ...
    const handleDetailProduct = (productId: string, productSlug?: string) => {
        // If we have a slug, navigate to the new product details page
        if (productSlug) {
            router.push(`/products/${productSlug}`);
        } else {
            // Fallback to old route if no slug available
        ...
        <>
            {type === "grid" ? (
                <div className={`product-item grid-type ${style}`}>
                    <div onClick={() => handleDetailProduct(data.id, data.slug)} className="product-main cursor-pointer block">
                            ...
                            <div className="product-img w-full h-full aspect-[3/4]">
                                {activeColor ? (
                                    <>
                                        {
                                            <Image
                                                src={data.variation.find(item => item.color === activeColor)?.image ?? data.coverImage ?? '/images/product/1000x1000.png'}
                                                width={500}
                                                height={500}
                                                alt={data.name}
                                                priority={true}
                                    ...
                                    <>
                                        ...
                                        <Image
                                            src={data.coverImage || data.thumbImage[0] || '/images/product/1000x1000.png'}
                                            width={500}
                                            height={500}
                                            priority={true}
                                            alt={data.name}
                                            className='w-full h-full object-cover duration-700'
                                            onError={(e) => {
                                                console.warn('Image failed to load:', data.coverImage || data.thumbImage[0]);
                                                setImageError(true);
                                            }}
                                        />
                                    </>
